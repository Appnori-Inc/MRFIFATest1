...
namespace BallPool.Mechanics
{
    public class Ball : MonoBehaviour
    {
        ...
        public Vector3 position{ get { return listener.body.position; } 
            set 
            {
...
//            return false;
        ...
        void Awake ()
        {
            ...
            initializeAudio(ballHitBall);

            ballHitBoard = gameObject.AddComponent<AudioSource>();
            ballHitBoard.playOnAwake = false;
            ballHitBoard.clip = ballHitBoardClip;
            initializeAudio(ballHitBoard);

            ballHitPocket = gameObject.AddComponent<AudioSource>();
            ballHitPocket.playOnAwake = false;
            ballHitPocket.clip = ballHitPocketClip;
            initializeAudio(ballHitPocket);

            //Appnori.Util.AudioMixerControl.OnInitialized += (instance) =>
            //{ 
            //    var targetGroup = instance.GetGroup((group) => group.name == Billiards.GameConfig.FxSoundGroupName);
            //    ballHitBall.outputAudioMixerGroup = targetGroup;
            //    ballHitBoard.outputAudioMixerGroup = targetGroup;
            //    ballHitPocket.outputAudioMixerGroup = targetGroup;
            //};
        ...
        void initializeAudio(in AudioSource source)
        {
            ...
            source.outputAudioMixerGroup = GameSettingCtrl.GetAudioMixerGroup("Effect");
        }
        ...
        void Update()
        {
            if(Billiards.LevelLoader.CurrentMatchInfo.gameType != Billiards.GameType.PocketBilliards)
            {
        ...
        Vector3 CalculateBallShadowPosition()
        {
            ...
            {
                var ballDownPosition = new Vector3(position.x, position.y - 0.5f * radius, position.z);
                return ballDownPosition + 0.5f * radius * (position - lightCentre.position);
            }
        }
        ...
        IEnumerator WaitAndPlayBallInPocket()
        {
            ...
            ballHitPocket.volume = Mathf.Clamp(10f * listener.normalizedVelocity.magnitude, 0.3f, 1.0f);
            ballHitPocket.Play();
        }
    }
}
