...
namespace Appnori
{
    public class TagsAndLayers : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            {
                if (!PropertyExists(tagsProp, 0, tagsProp.arraySize, tags[i]))
                    NewTag(tags[i]);
            }

            for (int i = 0; i < layers.Count; i++)
            {
                if (!PropertyExists(layersProp, 0, maxLayers, layers[i]))
                    NewLayer(layers[i]);
            }
            ...
            for (int i = 0; i < tagsProp.arraySize; i++)
            {
                ...
                int index = tags.FindIndex(x => x == t.stringValue);

                if (index == -1)
                    RemoveTag(t.stringValue);
            }
            ...
            for (int i = 0; i < layersProp.arraySize; i++)
            {
                ...
                int index = layers.FindIndex(x => x == l.stringValue);

                if (index == -1)
                    RemoveLayer(l.stringValue);
            }
        }
        private void Awake()
        {
