...
public class LobbySetting : MonoBehaviour
{
    ...
    public class JsonParsing
    {
        ...
        {
            string orizin_csv = File.ReadAllText(Application.dataPath + "/03. Individual Resources/Lee Jaeheung/Sheets/" + csvName);
    ...
    public class SettingInfo
    {
        ...
        public Notifier<float> value_haptic = new Notifier<float>();
    ...
    private XRController _controller1;
    private XRController _controller2;
    void Start()
    {
        Init();
    }
    ...
    private void Init()
    {
        ...
        isInit = true;
        _controller1 = GameObject.Find("XR Origin/Camera Offset/LeftHand Controller").GetComponent<XRController>();
        _controller2 = GameObject.Find("XR Origin/Camera Offset/RightHand Controller").GetComponent<XRController>();
        audioMixer = Resources.Load<AudioMixer>("MasterAudioMixer");

        Transform findTr = transform.Find("Window_Setting/Content/Slot (0)");
        ...
        findTr = transform.Find("Window_Setting/Content/Slot (1)");
        ...
        findTr = transform.Find("Window_Setting/Content/Slot (2)");
        ...
        findTr = transform.Find("Window_Setting/Content/Slot (3)");
        ...
        findTr = transform.Find("Window_Setting/Content/Slot (4)");
       ...
       /* EventTrigger.Entry entry = new EventTrigger.Entry();
        ...
        entry.callback.AddListener(delegate { PublicGameUIManager.GetInstance.Play_Voice(7); });
        ...
        InitSettingInfo();
        settingInfo.value_haptic.OnDataChanged += SetHaptic;

        InitLocalizationInfo();

        languageState = GetChangeLanguageState(settingInfo.language);

        if (StaticLocalizationCtrl.IsInstance)
        {
            StaticLocalizationCtrl.GetInstance.SetLocaliztion();
        }
        if (StaticLocalizationCtrl_TextMesh.IsInstance)
        {
            StaticLocalizationCtrl_TextMesh.GetInstance.SetLocaliztion();
        }
        if (StaticResourcesChangeCtrl.IsInstance)
        {
            StaticResourcesChangeCtrl.GetInstance.SetData();
        }
    ...
    public void View()
    {
        ...
        settingInfo.value_haptic.Value = Mathf.Clamp(settingInfo.value_haptic.Value, 0f, 2f);

        if (settingInfo.on_haptic)
        {
            toggle_haptic.isOn = true;
            slider_haptic.value = settingInfo.value_haptic.Value;
        }
        else
        {
    ...
    public void SetOnHaptic(bool isOn)
    {
        ...
        {
            slider_haptic.value = settingInfo.value_haptic.Value;
            GameDataManager.instance.userInfo_mine.valueHaptic = settingInfo.value_haptic.Value;
        }
        else
        {
            slider_haptic.value = 0f;
            GameDataManager.instance.userInfo_mine.valueHaptic = 0;
        }

        UnityEngine.XR.Interaction.Toolkit.XRController.value_haptic = GetHapticValue();

        _controller1.SendHapticImpulse(GetHapticValue(), 0.5f);
        _controller2.SendHapticImpulse(GetHapticValue(), 0.5f);
        isChangeData = true;
    ...
    public void SetValueHaptic(float value)
    {
        ...
        if (settingInfo.value_haptic.Value == value)
        {
            return;
        }

        settingInfo.value_haptic.Value = value;
        GameDataManager.instance.userInfo_mine.valueHaptic = value;
    ...
    public float GetHapticValue()
    {
        ...
        if (settingInfo.on_haptic)
        {
            return settingInfo.value_haptic.Value;
        }
        else
        {
            return 0f;
    ...
    private void SetHaptic(float i)
    {
        TrySendHaptic(_controller1.inputDevice, settingInfo.value_haptic.Value, 0.1f); // 손에 진동 주는 코드
    ...
    {
        if (File.Exists(Application.dataPath + "/03. Individual Resources/Lee Jaeheung/Sheets/Sheet_Localization.csv"))
        {
            string jsonString = JsonUtility.ToJson(new JsonParsing("Sheet_Localization.csv"));

            File.WriteAllText(Application.dataPath + "/03. Individual Resources/Lee Jaeheung/Resources/ItemInfos/LocalizationData.json", jsonString);
