...
**/
void MainLight_half(float3 worldPos, out half3 lightDir, out half3 lightColor0, out half distanceAtten, out half shadowAtten, out half testAtten)
{
#if SHADERGRAPH_PREVIEW			//! 쉐이더 그래프 프리뷰에서 보이는 결과
	lightDir = half3(0, 1, 0);
	lightColor0 = 1;
	distanceAtten = 1;
	shadowAtten = 1;
	testAtten = 1;
   ...
   Light mainLight = GetMainLight(shadowCoord);			//! Lighting.hlsl 함수로 라이트 정보 및 쉐도우 데이터 구조체 얻어옴
   ...
   testAtten = mainLight.shadowAttenuation;

   ShadowSamplingData shadowSamplingData = GetMainLightShadowSamplingData();	//! 쉐도우 감쇠값
   // half shadowsStrength = GetMainLightShadowStrength();
   half4 shadowParams = GetMainLightShadowParams();
   // shadowAtten = SampleShadowmap(shadowCoord,TEXTURE2D_ARGS(_MainLightShadowmapTexture, sampler_MainLightShadowmapTexture), TransformWorldToShadowCoord(worldPos), shadowSamplingData, shadowsStrength, false)
