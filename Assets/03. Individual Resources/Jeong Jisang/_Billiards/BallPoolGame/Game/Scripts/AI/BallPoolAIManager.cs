...
namespace BallPool.AI
{
    ...
    public abstract class BallPoolAIManager : MonoBehaviour
    {
        ...
        private void CalculateAI(int calculateMaxCount, Ball cueBall, float impulse, FindBestTargetBallException<int> FindException, bool changePosition)
        {
            ...
            if (cueBall.position.y < 0)
            {
                cueBall.position = cueBall.position.ToXZ().ToVector3FromXZ(cueBall.radius);
            }

            float newImpulse = impulse;
            if(!AightBallPoolGameLogic.gameState.tableIsOpened)
            {
                newImpulse = Mathf.Lerp(impulse, Random.Range(0.5f, 1.2f) * impulse, ((float)BallPoolAIExtension.DumbLevel).RemapClamped((1, 5), (0, 1)));
            }
            ...
            if(haveExaption)
            {
                info = FindSuboptimalTargetBall(cueBall, newImpulse, FindException, changePosition);
                haveExaption = !info.targetBall || FindException(info.targetBall.id);
            }
        ...
        private BestTargetBallInfo FindBestTargetBall(Ball cueBall, float impulse, FindBestTargetBallException<int> FindException, bool changePosition)
        {
            ...
            Ball ball = BallAICalculator.FindBestTargetBall(cueBall, targets, gameManager.balls, out pocketId, out shotBallPosition, out shotPoint, out aimpoint, ref impulse, FindException, changePosition, shotController.ballLayer, shotController.boardLayer, shotController.ClothSpace);
            return new BestTargetBallInfo(ball, pocketId, shotBallPosition, shotPoint, aimpoint, impulse).ToDumb();
        }
        ...
        private BestTargetBallInfo FindSuboptimalTargetBall(Ball cueBall, float impulse, FindBestTargetBallException<int> FindException, bool changePosition)
        {
            ...
            Vector3 shotPoint = Vector3.zero;
            Vector3 aimpoint = Vector3.zero;
            Ball ball = BallAICalculator.FindSuboptimalTargetBall(cueBall, targets, gameManager.balls, out pocketId, out shotBallPosition, out shotPoint, out aimpoint, ref impulse, FindException, changePosition, shotController.ballLayer, shotController.boardLayer);
        ...
        private BestTargetBallInfo FindSecondWorstTargetBall(Ball cueBall, float impulse, FindBestTargetBallException<int> FindException, bool changePosition)
        {
            ...
            Vector3 shotPoint = Vector3.zero;
            Vector3 aimpoint = Vector3.zero;
            Ball ball = BallAICalculator.FindSecondWorstTargetBall(cueBall, targets, gameManager.balls, out pocketId, out shotBallPosition, out shotPoint, out aimpoint, ref impulse, FindException, changePosition, shotController.ballLayer, shotController.boardLayer);
            return new BestTargetBallInfo(ball, pocketId, shotBallPosition, shotPoint, aimpoint, impulse);
        }
        ...
        public BestTargetBallInfo GetCalculateInfo(Ball cueBall, float impulse, FindBestTargetBallException<int> exception = null)
        {
            ...
            var info = FindBestTargetBall(cueBall, impulse, exception, false);
            var hasException = !info.targetBall || exception(info.targetBall.id);

            if (hasException)
            {
                info = FindSuboptimalTargetBall(cueBall, impulse, exception, false);
                hasException = !info.targetBall || exception(info.targetBall.id);
            }

            if (hasException)
            {
                info = FindSecondWorstTargetBall(cueBall, impulse, exception, false);
                hasException = !info.targetBall || exception(info.targetBall.id);
            }

            if (hasException)
            {
                Debug.Log("[BPAM] CANNOT found target ball.");
#if UNITY_EDITOR
