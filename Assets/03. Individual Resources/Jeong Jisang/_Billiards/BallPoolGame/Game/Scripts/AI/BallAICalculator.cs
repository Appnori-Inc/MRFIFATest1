    ...
    public struct BallAICalculator
    {
        public static Ball FindSecondWorstTargetBall(Ball cueBall, PocketListener[] targets, Ball[] balls, out int pocketId, out Vector3 cueBallPosition, out Vector3 shotPoint, out Vector3 aimpoint, ref float impulse, FindBestTargetBallException<int> FindException, bool changePosition, int ballsLayer, int boardLayer)
        {
            ...
            foreach (Ball ball in balls)
            {
                ...
                var distance = Vector3.Distance(targetPoint, cueBallPosition);
                if (maxBallDistance > distance)
                {
                    continue;
                }
        ...
        public static Ball FindSuboptimalTargetBall(Ball cueBall, PocketListener[] targets, Ball[] balls, out int pocketId, out Vector3 cueBallPosition, out Vector3 shotPoint, out Vector3 aimpoint, ref float impulse, FindBestTargetBallException<int> FindException, bool changePosition, int ballsLayer, int boardLayer)
        {
            ...
            foreach (Ball ball in balls)
            {
                ...
                if (hitAngleCosine <= currentHitAngleCosine || (currentHitAngleCosine > 0.5f))
                {
                    ball.isCast = false;
                    ...
                    if (isCast && ball.listener.transform == targetHit.transform)
                    {
                        isCast = false;
                    }
                        ...
                        if (hitAngleCosine < currentHitAngleCosine || (currentHitAngleCosine > 0.5f && targetBallToTargetDistance > currentTargetBallToTargetDistance))
                        {
                            ...
                            if (isCast)
                            {
                                ...
                                else
                                {
                                    ...
                                    //Vector3 newPosition = Geometry.ClampPositionInCube(targetPoint - displacement * cueBall.radius * targetBallShotDirection, cueBall.radius, clothSpace);
                                    ...
                                    while (isCast && displacement >= 4.0f)
                                    {
                                        ...
                                        //2021.8.10. bugfix : when displacement similarly 4, cueball posiion is outsied of table.
                                        //So Clamp calculate posiiton
                                        newPosition = Geometry.ClampPositionInCube(targetPoint - displacement * cueBall.radius * targetBallShotDirection, cueBall.radius, clothSpace);
                                        ...
                                        isCast = Physics.SphereCast(newPosition, cueBall.radius, targetBallShotDirection, out targetHit, Vector3.Distance(newPosition, targetPoint), ballsLayer) ||
                                            ...
                                            Physics.SphereCast(newPosition + 4.0f * cueBall.radius * Vector3.up, 1.5f * cueBall.radius, -Vector3.up, out targetHit, (6.0f * cueBall.radius * Vector3.up).magnitude, ballsLayer);

#if UNITY_EDITOR       
                                        //TESTCODE :: cull ball set overlapping position Exception
                                        //if (!Physics.SphereCast(newPosition + 4.0f * cueBall.radius * Vector3.up, 1.5f * cueBall.radius, -Vector3.up, out targetHit, Vector3.Distance(newPosition, targetPoint), ballsLayer))
                                        //{
                                        //    var testCast = Physics.SphereCast(newPosition + 4.0f * cueBall.radius * Vector3.up, 1.5f * cueBall.radius, -Vector3.up, out targetHit, (6.0f * cueBall.radius * Vector3.up).magnitude, ballsLayer);

                                            //if (testCast)
                                            //{
                                            //    UnityEditor.EditorApplication.isPaused = true;
                                            //    var instance = new GameObject();
                                            //    instance.name = $"testCast({Vector3.Distance(newPosition, targetPoint)})";
                                            //    instance.transform.position = newPosition + 4.0f * cueBall.radius * Vector3.up;


                                            //    instance = new GameObject();
                                            //    instance.name = $"targetPoint()";
                                            //    instance.transform.position = targetPoint;
                                            //}
                                        //}
#endif
                                        ...
                                        if (!isCast)
                                        {
                                            ...
                                            cueBallPosition = newPosition.ToXZ().ToVector3FromXZ(cueBallPosition.y);
                                            targetPoint = ball.position - (cueBall.radius + ball.radius) * targetBallShotDirection;
                                            shotDirection = Vector3.ProjectOnPlane(targetPoint - cueBallPosition, Vector3.up).normalized;
                                            shotPoint = cueBallPosition - cueBall.radius * shotDirection;
                                            aimpoint = targetPoint;
...
#if UNITY_EDITOR
                                    //TESTCODE :: cull ball set Out of Table Exception
                                    //var testPosition = Geometry.ClampPositionInCube(cueBallPosition, cueBall.radius, clothSpace);
                                    //if (!isCast && testPosition.ToXZ() != cueBallPosition.ToXZ())
                                    //{
                                    //    UnityEditor.EditorApplication.isPaused = true;
                                    //    var instance = new GameObject();
                                    //    instance.name = "cueBallPosition";
                                    //    instance.transform.position = cueBallPosition;

                                    //    instance = new GameObject();
                                    //    instance.name = "ClampedPositon";
                                    //    instance.transform.position = testPosition;
                                    //}
#endif
        ...
        private static bool CheckBoardl(Vector3 direction, float cueBallRadius, Vector3 cueBallPosition, Vector3 targetPoint, BallListener targetBall, ref Vector3 aimpoint, ref float hitAngleCosine, ref float targetBallToTargetDistance, int ballsLayer, int boardLayer)
        {
            ...
            if(Physics.SphereCast(ray, cueBallRadius, out hitBoard, 5.0f, boardLayer))
            {
                ...
                {
                    if(checkPointHit.collider.gameObject.layer == Appnori.Layer.NameToLayer( Appnori.Layer.GameType.Billiards,"Board"))
                    {
