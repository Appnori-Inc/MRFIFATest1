...
//                }
                ...
                if (inMove)
                {
                    ...
                    if (!BallPoolGameLogic.controlFromNetwork)
                    {
                        if (CheckIsSleeping(false))
                        {
                            ...
                            {
                                yield return StartCoroutine(nameof(StopMove));
                            }
                            else
                            {
                                StopCoroutine(nameof(StopMove));
                            }
        ...
        private IEnumerator StopMove()
        {
            ...
            yield return new WaitForSeconds(GameConfig.TurnEndDelay);
        ...
        {
            float minEnergy = GameConfig.MinBallMoveEnergy;
            bool isSleep = true;
            foreach (BallListener ball in ballsListener)
            {
